...
@section('content')
    <div class="animate-fade-in mx-auto max-w-2xl space-y-5">
        <section class="flex items-center justify-between gap-3">
            <div>
                <h2 class="font-display text-xl font-bold text-slate-800">Edit User</h2>
                <p class="text-sm text-slate-500">Perbarui informasi akun <span class="font-semibold text-slate-700">{{ $user->name }}</span>.</p>
            </div>
        ...
        <div class="card p-6">
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <label for="password" class="form-label">Password <span class="text-xs font-normal text-slate-400">(kosongkan jika tidak diubah)</span></label>
